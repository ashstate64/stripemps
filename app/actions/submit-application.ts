...
export async function getFormSubmitApiKey(email: string): Promise<{ success: boolean; message: string; apiKey?: string }> {
  try {
    const response = await fetch(`https://formsubmit.co/api/get-apikey/${encodeURIComponent(email)}`, {
      method: 'GET',
      headers: {
    ...
    if (data.success) {
      return {
        success: true,
        message: 'API key request sent to your email. Please check your mailbox and update your environment variables.',
        apiKey: data.apiKey
      };
    } else {
      return {
...
export async function getFormSubmissions(): Promise<{ success: boolean; submissions?: any[]; message: string }> {
  if (!FORMSUBMIT_API_KEY) {
    return {
      success: false,
      message: 'API key not configured. Please set FORMSUBMIT_API_KEY environment variable.'
    };
  }

  try {
    const response = await fetch(`https://formsubmit.co/api/get-submissions/${FORMSUBMIT_API_KEY}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
...
export async function submitApplication(
  ...
  try {
    ...
    const submissionId = `OAI-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`.toUpperCase();
    ...
    return {
      message: 'An unexpected error occurred. Please try again later or contact our support team at accounts@cgfinancialcanada.ca',
      success: false,
    };
  }
}
...
export async function testFormSubmitConnection(): Promise<{ success: boolean; message: string }> {
  try {
    const testData = {
      ...
      message: 'This is a connection test for the OpenAI investment application form.',
