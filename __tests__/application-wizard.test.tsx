...
describe('ApplicationWizard', () => {
  ...
  it('advances to next step when Next button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Step 2 of 5: Employment & Financial Profile')
      ).toBeInTheDocument();
    });
  });
  ...
  it('shows Review & Submit button on final step', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Step 5 of 5: Review & Submit')
      ).toBeInTheDocument();
      expect(
        screen.getByRole('button', { name: /review & submit application/i })
      ).toBeInTheDocument();
    });
  });
