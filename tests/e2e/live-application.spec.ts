...
test.describe('Live OpenAI Application Form - Production Tests', () => {
  ...
  test('should load the main application page', async ({ page }) => {
    ...
    // Check main heading
    await expect(
      page.getByRole('heading', { name: /Invest in the Future of AI/i })
    ).toBeVisible();

    // Check key metrics are visible - target specific metric cards
    await expect(
      page.locator('.grid .group').filter({ hasText: '~$300B' }).first()
    ).toBeVisible();
    await expect(
      page.locator('.grid .group').filter({ hasText: '$58B+' }).first()
    ).toBeVisible();
    await expect(
      page.locator('.grid .group').filter({ hasText: 'Q4 2025' }).first()
    ).toBeVisible();
  });
  ...
  test('should navigate to application form', async ({ page }) => {
    ...
    await page
      .getByRole('link', { name: /start investment application/i })
      .click();
    ...
    await expect(
      page.getByRole('heading', {
        name: /OpenAI Pre-IPO Investment Application/i,
      })
    ).toBeVisible();
  });
  ...
  test('should display application form correctly', async ({ page }) => {
    ...
    await expect(
      page.getByText('Step 1 of 5: Personal Information')
    ).toBeVisible();
    ...
    await expect(
      page.getByText('Secure • Encrypted • Confidential')
    ).toBeVisible();
  });
  ...
  test('should have working navigation buttons', async ({ page }) => {
    ...
    await expect(
      page.getByText('Step 2 of 5: Employment & Financial Profile')
    ).toBeVisible();
  ...
  test('should display correct updated dates (Q4 2025)', async ({ page }) => {
    ...
    await expect(
      page
        .locator('[class*="text-purple-400"]')
        .filter({ hasText: 'Q4 2025' })
        .first()
    ).toBeVisible();
    await expect(
      page.locator('span').filter({ hasText: /Target close: Q4 2025/i })
    ).toBeVisible();
  ...
  test('should display legal disclaimers', async ({ page }) => {
    ...
    await expect(
      page.getByText(/this application does not constitute an offer/i)
    ).toBeVisible();
    await expect(
      page.getByText(/investment opportunities are subject to availability/i)
    ).toBeVisible();
