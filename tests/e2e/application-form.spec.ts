...
test.describe('OpenAI Investment Application Form', () => {
  ...
  test('should have Previous button disabled on first step', async ({
    page,
  }) => {
  ...
  test('should navigate through all steps', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /next/i }).click();
    await expect(
      page.getByText('Step 3 of 5: Accredited Investor Status')
    ).toBeVisible();
    ...
    await expect(
      page.getByRole('button', { name: /review & submit application/i })
    ).toBeVisible();
  });
  ...
  test('should fill out personal information form', async ({ page }) => {
    ...
    await expect(page.getByLabel(/phone number/i)).toHaveValue(
      '+1-555-123-4567'
    );
  });
  ...
  test('should display security information', async ({ page }) => {
    ...
    await expect(
      page.getByText(/protected by industry-standard ssl encryption/i)
    ).toBeVisible();
  });
