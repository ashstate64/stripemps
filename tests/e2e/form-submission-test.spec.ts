...
test.describe('Form Submission Test - Email Delivery Verification', () => {
  ...
  test('should complete full application submission and send email', async ({
    page,
  }) => {
    ...
    await page
      .getByLabel(/net financial assets/i)
      .selectOption('$1,000,000 - $5,000,000');
    ...
    await page
      .getByRole('button', { name: /review & submit application/i })
      .click();
    ...
    await expect(
      page.locator('[class*="text-green-400"]').filter({ hasText: /OAI-/i })
    ).toBeVisible();
